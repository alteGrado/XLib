...
Window create_simple_window (Display* display, int width, int height, int x, int y)
{
    ...
    window = XCreateSimpleWindow(display, RootWindow(display, scr_number), x, y, width, height,
                                  window_border_width, BlackPixel(display, scr_number),
...
int main (int argc, char* argv[])
{
    ...
    if (display == NULL)    {
	fprintf (stderr, "%s: I can not connect to the X server '%s'\n",
                 argv[0], display_name);
        exit(1);
    }
    ...
    /*создаем простое окно как прямой потомок корневого окна экрана,
      ...
      размещая новое окно в левом верхнем углу по заданным координатам*/
	...
	XQueryTree (display, window,&root_window,&parent_window,
        ...
        /*Транслируем локальные координаты в экранные, используя
          корневое окно как окно, относительно которого выполняется трансляция.
          Это работает потому, что корневое окно занимает всегда весь экран,
          и его верхний левый угол совпадает с левым верхним углом экрана*/
        XTranslateCoordinates(display, parent_window, window_attr.root,
                              x, y, &scr_x, &scr_y, &child_window);
